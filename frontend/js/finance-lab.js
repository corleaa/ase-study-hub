...
function financeStressSvg(metrics) {
  ...
  var isStability = metrics.finance.mode === 'stability';
  var axisColor = isStability ? 'rgba(42,57,78,0.62)' : 'rgba(255,255,255,0.16)';
  var labelColor = isStability ? '#203149' : 'rgba(255,255,255,0.62)';
  ...
  if (isStability) {
    ...
    svg += '<text x="' + (pad + 12) + '" y="54" text-anchor="start" fill="rgba(62,118,95,1)" font-size="13" font-weight="900">Zonă sigură</text>';
    svg += '<text x="' + (pad + 12) + '" y="108" text-anchor="start" fill="rgba(170,108,38,1)" font-size="13" font-weight="900">Supraveghere</text>';
    svg += '<text x="' + (pad + 12) + '" y="164" text-anchor="start" fill="rgba(180,56,74,1)" font-size="13" font-weight="900">Zonă critică</text>';
  }
  svg += '<line x1="' + pad + '" y1="' + baseY + '" x2="' + (width - pad) + '" y2="' + baseY + '" stroke="' + axisColor + '" stroke-width="1"/>';
  if (isStability) {
    svg += '<line x1="' + pad + '" y1="140" x2="' + (width - pad) + '" y2="140" stroke="rgba(180,56,74,0.38)" stroke-width="1.5" stroke-dasharray="5 6"/>';
    svg += '<line x1="' + pad + '" y1="104" x2="' + (width - pad) + '" y2="104" stroke="rgba(170,108,38,0.28)" stroke-width="1.5" stroke-dasharray="5 6"/>';
  }
  items.forEach(function(item, idx) {
    ...
    svg += '<text class="fin-anim-fade" x="' + (x + barW / 2) + '" y="' + (baseY + 22) + '" text-anchor="middle" fill="' + labelColor + '" font-size="12" font-weight="800">' + item.label + '</text>';
  });
  return svg + '</svg>';
}
...
function financeStabilitySvg(metrics) {
  ...
  var isStability = metrics.finance.mode === 'stability';
  var ringColor = isStability ? 'rgba(39,55,76,0.34)' : 'rgba(255,255,255,0.08)';
  var axisColor = isStability ? 'rgba(33,47,66,0.58)' : 'rgba(255,255,255,0.12)';
  var labelColor = isStability ? '#27384e' : 'rgba(255,255,255,0.6)';
  var axes = [
  ...
  axes.forEach(function(axis, idx) {
    ...
    svg += '<line x1="' + cx + '" y1="' + cy + '" x2="' + ex.toFixed(2) + '" y2="' + ey.toFixed(2) + '" stroke="' + axisColor + '" stroke-width="1.5"/>';
    svg += '<text x="' + (cx + Math.cos(angle) * (radius + 30)).toFixed(2) + '" y="' + (cy + Math.sin(angle) * (radius + 30)).toFixed(2) + '" text-anchor="middle" fill="' + labelColor + '" font-size="12.5" font-weight="800">' + axis.label + '</text>';
    if (isStability) {
      svg += '<text x="' + (cx + Math.cos(angle) * (radius + 30)).toFixed(2) + '" y="' + (cy + Math.sin(angle) * (radius + 44)).toFixed(2) + '" text-anchor="middle" fill="rgba(53,69,91,0.96)" font-size="11.5" font-weight="800">' + Math.round(axis.value) + '/100</text>';
    }
    if (isStability) svg += '<circle cx="' + px.toFixed(2) + '" cy="' + py.toFixed(2) + '" r="5.5" fill="#f29b6d" stroke="#243348" stroke-width="2.4"/>';
  });
  svg += '<path d="' + polygon + ' Z" fill="rgba(242,155,109,0.22)" stroke="' + (isStability ? '#243348' : '#f29b6d') + '" stroke-width="3.8"/>';
  if (isStability) {
...
function financePolicySvg(metrics) {
  ...
  svg += '<text x="' + (padL + 8) + '" y="56" text-anchor="start" fill="rgba(65,118,95,1)" font-size="13" font-weight="900">Buffere confortabile</text>';
  svg += '<text x="' + (padL + 8) + '" y="98" text-anchor="start" fill="rgba(177,112,38,1)" font-size="13" font-weight="900">Calibrare atentă</text>';
  svg += '<text x="' + (padL + 8) + '" y="154" text-anchor="start" fill="rgba(186,59,78,1)" font-size="13" font-weight="900">Intervenție necesară</text>';
  for (var g = 0; g <= 4; g++) {
    var gy = padT + ((height - padT - padB) / 4) * g;
    svg += '<line x1="' + padL + '" y1="' + gy + '" x2="' + (width - padR) + '" y2="' + gy + '" stroke="rgba(86,102,122,0.22)" stroke-width="1"/>';
    svg += '<text x="' + (padL - 6) + '" y="' + (gy + 4) + '" text-anchor="end" fill="rgba(81,98,120,0.88)" font-size="10.5" font-weight="700">' + ((maxVal - (maxVal / 4) * g).toLocaleString('ro-RO', { maximumFractionDigits: 1 })) + '%</text>';
  }
  items.forEach(function(item, idx) {
    ...
    if (item.label !== 'MREL') {
      svg += '<text class="fin-anim-fade" x="' + (x + barW / 2) + '" y="' + (height - 34) + '" text-anchor="middle" fill="rgba(73,89,110,0.92)" font-size="10.5" font-weight="700">buffer</text>';
    } else {
      svg += '<text class="fin-anim-fade" x="' + (x + barW / 2) + '" y="' + (height - 34) + '" text-anchor="middle" fill="rgba(73,89,110,0.92)" font-size="10.5" font-weight="700">resoluție</text>';
    }
    svg += '<text class="fin-anim-fade" x="' + (x + barW / 2) + '" y="' + (height - 8) + '" text-anchor="middle" fill="rgba(46,60,80,0.96)" font-size="12.5" font-weight="800">' + item.label + '</text>';
...
function financeStabilityBreakPoint(metrics) {
  var channels = [
    { label: 'capitalul CET1', severity: financeClamp(100 - metrics.capitalRatio * 5, 0, 100) },
    { label: 'lichiditatea LCR', severity: financeClamp((120 - metrics.liquidityCoverage) * 1.2, 0, 100) },
    { label: 'excesul de credit (gap proxy)', severity: financeClamp(metrics.creditGapStress * 8.5, 0, 100) },
    { label: 'contagiunea între segmente', severity: financeClamp(metrics.contagionPressure, 0, 100) }
  ].sort(function(a, b) { return b.severity - a.severity; });
  return channels[0];
}

function financePolicyPriority(metrics) {
  if (metrics.ccybNeed >= 1.5) return 'acumularea de buffer ciclic';
  if (metrics.nplRate >= 6 || metrics.contagionPressure >= 45) return 'întărirea amortizoarelor structurale';
  if (metrics.liquidityCoverage < 110) return 'apărarea lichidității și a refinanțării';
  return 'monitorizare și calibrare atentă';
}
...
function updateFinanceLabUI() {
  ...
  var strips = {
    ...
    finStressStrip: isStability
      ? '<div class="finance-stat-chip"><strong>Punct de ruptură</strong><span>' + financeStabilityBreakPoint(metrics).label + '</span></div><div class="finance-stat-chip"><strong>Severitate</strong><span>' + financeRiskLevelMeta(financeStabilityBreakPoint(metrics).severity).label + '</span></div><div class="finance-stat-chip"><strong>CET1 / LCR</strong><span>' + financePct(metrics.capitalRatio, 1) + ' / ' + financePct(metrics.liquidityCoverage, 0) + '</span></div><div class="finance-stat-chip"><strong>Gap credit / CCyB</strong><span>' + financePct(metrics.creditGapStress, 1) + ' / ' + financePct(metrics.ccybNeed, 1) + '</span></div>'
    ...
    finStabilityStrip: isStability
      ? '<div class="finance-stat-chip"><strong>Prioritate de politică</strong><span>' + financePolicyPriority(metrics) + '</span></div><div class="finance-stat-chip"><strong>Risc sistemic / absorbție</strong><span>' + Math.round(metrics.systemicRiskScore) + '/100 · ' + Math.round(metrics.shockAbsorption) + '/100</span></div><div class="finance-stat-chip"><strong>Fragilitate housing</strong><span>' + Math.round(metrics.housingFragility) + '/100</span></div><div class="finance-stat-chip"><strong>Scor stabilitate</strong><span>' + Math.round(metrics.stabilityScore) + '/100</span></div>'
